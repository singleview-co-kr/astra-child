...
function hide_settings_page($query) {
    ...
    if ($typenow === "page") {
        ...
        $settings_page_id2 = get_page_by_path("케이보드-추천-상품-설정", NULL, "page")->ID;
        $settings_page_id3 = get_page_by_path("메인-블로그태그-설정", NULL, "page")->ID;

        // 쿼리에서 이 페이지들을 제외합니다.
        $query->set('post__not_in', array($settings_page_id1, $settings_page_id2, $settings_page_id3));
    }
    return;
}
...
function add_site_settings_to_menu()
{
    ...
    add_menu_page('케이보드 추천상품 설정', '케이보드 추천상품 설정', 'manage_options', 'post.php?post=' . get_page_by_path("케이보드-추천-상품-설정", NULL, "page")->ID . '&action=edit', '', 'dashicons-admin-tools', 20);
...
function higlight_custom_settings_page($file) {
    ...
    $post = (int)$_GET["post"];
    if ($pagenow === "post.php" && ($post === $settings_page_id1 || $post === $settings_page_id2 || $post === $settings_page_id3)) {
